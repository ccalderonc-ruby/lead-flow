...
module Admin
  class UsersController < InertiaController
    ...
    def demoting_self?(attrs)
      ...
      new_role.present? && new_role.name != "admin"
    end
    ...
    def would_leave_zero_active_admins?(attrs)
      ...
      new_role = Role.find_by(id: new_role_id)
      still_active_admin = new_status.to_s != "disabled" && new_role&.name == "admin"
    ...
    def form_props(user)
      {
        ...
        roles: Role.order(:name).map { |role| { id: role.id, name: role.name } },
