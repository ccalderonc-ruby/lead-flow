...
export default function ThemeToggle({ className = '' }: { className?: string }) {
  ...
  return (
    <button
      ...
      onClick={toggle}
      className={[
        'inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-panel px-3 py-1.5 text-sm font-medium text-slate-700 transition-colors hover:bg-slate-50',
        className,
      ]
        .filter(Boolean)
        .join(' ')}
